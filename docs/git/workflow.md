...
| Type | 说明 | 示例 |
|------|------|------|
| `feat` | 新功能 | feat: 添加用户评论功能 |
| `fix` | Bug修复 | fix: 修复登录按钮无响应 |
| `docs` | 文档更新 | docs: 更新API文档 |
| `style` | 代码格式（不影响逻辑） | style: 统一代码缩进 |
| `refactor` | 重构（不是新功能也不是修复） | refactor: 重构用户认证模块 |
| `perf` | 性能优化 | perf: 优化数据库查询 |
| `test` | 测试相关 | test: 添加单元测试 |
| `build` | 构建系统或外部依赖 | build: 升级webpack到5 |
| `ci` | CI配置 | ci: 添加GitHub Actions |
| `chore` | 其他不修改src或测试文件 | chore: 更新.gitignore |
| `revert` | 回退提交 | revert: 撤销feat/add-login |
...
详细描述本次提交的内容，**每行不超过72个字符**，**解释"为什么"而不是"是什么"**：
...
BREAKING CHANGE: 用户接口返回格式已更改
- username字段改为user_name
- 移除了age字段
```
...
npm install -D @semantic-release/changelog @semantic-release/npm
...
# 示例
feat(git): 添加Conventional Commits规范说明
fix(workflow): 修复分支合并冲突问题
docs(readme): 更新项目介绍
```
